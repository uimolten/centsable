...
// These functions DO NOT need secrets.
exports.adminGetAllUsers = onCall(async (request) => {
  await verifyAdmin(request);
  const userRecords = await admin.auth().listUsers();
  return userRecords.users.map((user) => ({ uid: user.uid, email: user.email }));
});
...
exports.adminResetAllUsers = onCall(async (request) => {
  ...
  usersSnapshot.forEach((doc) => {
    batch.update(doc.ref, { xp: 0, cents: 0, quests: [], completedLessons: [] });
  });
  await batch.commit();
...
exports.getOrGenerateDailyQuests = onCall({ secrets: ["GEMINI_API_KEY"] }, async (request) => {
  // Access the secret like this if you need it: const myApiKey = process.env.GEMINI_API_KEY;
  if (!request.auth) { throw new onCall.HttpsError('unauthenticated', 'Authentication required.'); }
  ...
  if (!lastReset || lastReset.getTime() < today5amPT.getTime()) {
    ...
    await userRef.update({ quests: newQuests, questsLastGenerated: admin.firestore.FieldValue.serverTimestamp() });
    return newQuests;
  }
